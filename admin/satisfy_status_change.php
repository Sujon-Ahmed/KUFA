...
$id = $_GET['id'];
$select_status = "SELECT * FROM `satisfies` WHERE `id` = $id";
...
if ($after_assoc_status['status'] == 1) {
    ...
    $update_status = "UPDATE `satisfies` SET `status`= 0 WHERE `id`=$id";
    $update_result = mysqli_query($db_connection, $update_status);
    header('location:view_satisfy.php');
} else {
    $select_total_status = "SELECT COUNT(*) AS `total_active` FROM `satisfies` WHERE `status` = 1";
    ...
    if ($after_assoc_active_status['total_active'] == 4) {
        Flash_data::error("You Can Active Maximum 4 Satisfies");
        header('location:view_satisfy.php');
    } else {
        // update status 0 to 1
        $update_status = "UPDATE `satisfies` SET `status`= 1 WHERE `id`=$id";
        $update_result = mysqli_query($db_connection, $update_status);
        header('location:view_satisfy.php');
    }
}
