...
$id = $_GET['id'];
$select_status = "SELECT * FROM `banners` WHERE `banner_id` = $id";
...
if ($after_assoc_status['banner_status'] == 1) {
    ...
    $update_status = "UPDATE `banners` SET `banner_status`= 0 WHERE `banner_id`=$id";
    ...
    $update_status = "UPDATE `banners` SET `banner_status`= 1 WHERE `banner_id`=$id";
    $update_result = mysqli_query($db_connection, $update_status);
    header('location:view_banner.php');
}
?>